...
src-tauri/src/
  ...
  probe_api/      The probe entry points, one file per cluster (replace/insert/inspect/ftsw/
                  ...
                  mod.rs owns SCRATCH_SLOTS = [400,401,402] — the ONE declaration every
...
- **`outputLevel`=0 is DEEP DIGITAL SILENCE on the real TMP, and `leveller::loudest_loudness` ERRORS ("no signal captured") on a silent capture** — **`outputLevel`=0 is DEEP DIGITAL SILENCE on the real TMP, and `leveller::loudest_loudness` ERRORS ("no signal captured") on a silent capture** — a finite LUFS isn't recoverable from silence. [→ evidence](notes/gotchas.md#outputlevel0-is-deep-digital-silence-on-the-real-tmp-and-levellerloudest_loudness-errors-no-signal-captured-on-a-silent-capture)
- **Re-amp engages reliably only ONCE per connection** — **Re-amp engages reliably only ONCE per connection.** Fresh-connect per engage. Its `ReAmpModeChanged` echo is flaky and is NOT proof of engagement — a finite captured loudness is. [→ evidence](notes/gotchas.md#re-amp-engages-reliably-only-once-per-connection)
- **A silent/failed re-amp inject reads as the device's STATIONARY OUTPUT FLOOR, and `measure_c` would accept it as a valid `C` without the production floor guards below** — **A silent/failed re-amp inject reads as the device's STATIONARY OUTPUT FLOOR, and `measure_c` would accept it as a valid `C` without the production floor guards below.** In a rapid 20-engage `probe --stim-ab` sweep, 19/20 captures measured the post-DSP floor, not the stimulus. [→ evidence](notes/gotchas.md#a-silentfailed-re-amp-inject-reads-as-the-devices-stationary-output-floor-and-measure_c-would-accept-it-as-a-valid-c-without-the-production-floor-guards-below)
- **Amp-id matching must be CHECK-FIRST then strip (CabIR/ConvRvb)** — **Amp-id matching must be CHECK-FIRST then strip (CabIR/ConvRvb).** A discovered amp block can carry merged cab/IR/convolution suffixes the catalog's bare bid lacks (e.g. [→ evidence](notes/gotchas.md#amp-id-matching-must-be-check-first-then-strip-cabirconvrvb)
...
- **CodeRabbit: progressive review is automatic — post NO command on a reviewed PR** — **CodeRabbit: progressive review is automatic — post NO command on a reviewed PR.** Pushing fix commits or replying to threads is enough; the incremental review picks them up on its own. [→ evidence](notes/gotchas.md#coderabbit-progressive-review-is-automatic-post-no-command-on-a-reviewed-pr)
- **Auto-merge arms ONLY for `main`-targeted PRs** — **Auto-merge arms ONLY for `main`-targeted PRs** (`.github/workflows/arm-auto-merge.yml`, since PR #52). [→ evidence](notes/gotchas.md#auto-merge-arms-only-for-main-targeted-prs)
...
- **Fully automatic connection** — **Fully automatic connection** — there are no manual Connect/Disconnect buttons. [→ evidence](notes/gotchas.md#fully-automatic-connection)
