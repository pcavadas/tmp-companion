...
test.describe("Songs — full CRUD (self-cleaning)", () => {
  ...
  test("create / rename / delete a song and create / delete a setlist", async ({ page }) => {
    await page.goto("/");
    ...
    await expect(page.getByText(SONG, { exact: true })).toBeVisible({ timeout: 30_000 });
    ...
    await page.getByTitle("Save").click();
    await expect(page.getByText(SONG2, { exact: true })).toBeVisible({ timeout: 30_000 });
    ...
    await expect(page.getByText(SONG, { exact: true })).toHaveCount(0, { timeout: 30_000 });
    ...
    await expect(page.getByText(SETLIST, { exact: true })).toBeVisible({ timeout: 30_000 });
    ...
    await expect(page.getByText(/reading this setlist/i)).toHaveCount(0, { timeout: 30_000 });
    await page.getByTitle("Setlist options").click();
    await page.getByText(/Delete setlist/).click();
    await page.getByRole("button", { name: "Delete", exact: true }).click();
    await expect(page.getByText(SETLIST, { exact: true })).toHaveCount(0, { timeout: 30_000 });
    ...
    await page.getByRole("button", { name: "Delete", exact: true }).click();
    await expect(page.getByText(SONG2, { exact: true })).toHaveCount(0, { timeout: 30_000 });
  });
});
