...
description: "The USB wire-protocol and `.preset` codec reference for the Tone Master Pro. Use whenever working with the wire format in `proto.rs` (the FenderMessageTMS codec + golden vectors), `session.rs` (handshake + commands) or `backup.rs` (the `.preset` XOR codec + library backup), or when adding or decoding a device message, a block-mutation op (insert/replace/remove), or a song/setlist setter. This is the wire-encoding reference; `notes/protocol.md` is the operational how-to and `CLAUDE.md` is the index. Bundled references carry the field tables and framing, the block-mutation wire shapes, the protocol lineage, and a `.preset` decode/encode/inspect toolkit."
---
