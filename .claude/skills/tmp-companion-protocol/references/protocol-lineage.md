...
| Source-of-truth           | "hardware is single source of truth, app is companion" (Medium post)                    | on a lean session, no unsolicited `CurrentPresetDataChanged`; a dense-heartbeat session DOES receive `PresetLoaded`/`SceneLoaded`/`CurrentPresetDataChanged` pushes on change (see `quick-reference.md`) | same philosophy                                                               |
