...
**A per-block Scene Edit flag also governs whether a parameter edit is scene-exclusive.** Independent of `sceneChangeBehavior`: `SetNodeSceneEdit` (wire) / a per-block flag (`ENABLED`, the default, means a parameter change applies only to the active scene; `DISABLED` means it's shared across all scenes). Any scene-vs-preset persistence experiment is only interpretable alongside this flag's state on the target block.
