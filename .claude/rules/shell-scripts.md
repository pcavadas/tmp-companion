...
**TEST under `/bin/bash` explicitly**, not the dev's newer PATH bash, which masks the failure. This bit `scripts/e2e.sh` twice: the default no-arg run aborted, and `online all` silently ran a single spec.
