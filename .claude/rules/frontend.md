...
- **Legit exception — DOM measurement.** A `useLayoutEffect` that measures the COMMITTED DOM (`getBoundingClientRect`/`getBBox`) then `setState`s the measurement is the one case the `setState` must stay in the effect (the node isn't laid out yet). Guard it with a prev-value compare so it converges. Instances: `SignalChainView`'s `SplitGroup` brackets, `ui/BlockArt.tsx` `HalfStackArt`, `overlays/usePickAnchor.ts` (the shared hook behind `Pick`/`FsParamPick`/`SceneLevelPick` — ONE effect, not three), `settings/TargetRow.tsx`.
