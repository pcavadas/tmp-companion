...
- **macOS ships; Linux also ships (alpha).** `core-foundation`/`core-foundation-sys` are `cfg(target_os = "macos")`; `libc` is pulled by BOTH macOS and Linux (`hid.rs`'s hidraw transport). The IOKit HID and cpal CoreAudio paths are macOS-only, but the crate builds and runs on Linux too — `hid.rs` has a hidraw `imp`, and `audio.rs` has an ALSA `hw:`-device `imp` (device resolution via `/proc/asound`, I32↔f32 conversion at the stream-callback boundary since the TMP's `hw:` interface has no F32 format). `release.yml` now builds a Linux `.deb`/`.rpm` alongside the macOS DMG (never blocking it) — see `notes/linux-packaging.md`.
